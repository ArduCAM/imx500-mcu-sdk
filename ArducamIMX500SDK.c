...
enum {
    I2C_PAYLOAD_DEFAULT_CHUNK_LEN = 4096,
    I2C_PAYLOAD_FALLBACK_CHUNK_LEN = 4,
    I2C_PAYLOAD_DRAIN_TIMEOUT_MS = 5000,
};
...
static void imx500_copy_fb_string(char *dst, size_t dst_size, flatbuffers_string_t src)
{
    if (!dst || dst_size == 0) {
        return;
    }
    ...
    size_t copy_len = imx500_min_size(dst_size - 1, strlen(src));
...
static bool imx500_tensor_parse_dimensions(IMX500ParsedTensor *dst, apParams_fb_FBDimension_vec_t dims)
{
    if (!dst || !dims) {
        return false;
    }

    dst->dimension_count = (uint8_t)imx500_min_size(IMX500_MAX_TENSOR_DIMS, apParams_fb_FBDimension_vec_len(dims));
    dst->element_count = 1;
    for (uint8_t i = 0; i < dst->dimension_count; ++i) {
        apParams_fb_FBDimension_table_t dim = apParams_fb_FBDimension_vec_at(dims, i);
        if (!dim || apParams_fb_FBDimension_size(dim) == 0) {
            return false;
        }
        dst->dimensions[i].id = (uint16_t)apParams_fb_FBDimension_id(dim);
        dst->dimensions[i].size = (uint16_t)apParams_fb_FBDimension_size(dim);
        dst->dimensions[i].serialization_index = (uint16_t)apParams_fb_FBDimension_serializationIndex(dim);
        dst->dimensions[i].padding = (uint16_t)apParams_fb_FBDimension_padding(dim);
...
static bool imx500_parse_tensor_common(IMX500ParsedTensor *dst,
                                       ...
                                       apParams_fb_FBDimension_vec_t dims)
{
    if (!dst) {
        return false;
    }
...
static bool imx500_parse_networks_from_ap_params(const uint8_t *ap_buf, uint32_t ap_len, IMX500ParsedMetadata *parsed_metadata)
{
    ...
    if (apParams_fb_FBApParams_verify_as_root(ap_buf, ap_len) != flatcc_verify_ok) {
        return false;
    }

    apParams_fb_FBApParams_table_t ap_parameter = apParams_fb_FBApParams_as_root(ap_buf);
    if (!ap_parameter) {
        return false;
    }

    apParams_fb_FBNetwork_vec_t networks = apParams_fb_FBApParams_networks(ap_parameter);
    if (!networks || apParams_fb_FBNetwork_vec_len(networks) == 0) {
        return false;
    }

    parsed_metadata->network_count = (uint8_t)imx500_min_size(IMX500_MAX_NETWORKS, apParams_fb_FBNetwork_vec_len(networks));
    ...
    for (uint8_t network_index = 0; network_index < parsed_metadata->network_count; ++network_index) {
        IMX500ParsedNetwork *dst_network = &parsed_metadata->networks[network_index];
        apParams_fb_FBNetwork_table_t src_network = apParams_fb_FBNetwork_vec_at(networks, network_index);
        if (!src_network) {
            return false;
        }

        memset(dst_network, 0, sizeof(*dst_network));
        dst_network->id = apParams_fb_FBNetwork_id(src_network);
        imx500_copy_fb_string(dst_network->name, sizeof(dst_network->name), apParams_fb_FBNetwork_name(src_network));
        imx500_copy_fb_string(dst_network->type, sizeof(dst_network->type), apParams_fb_FBNetwork_type(src_network));

        apParams_fb_FBInputTensor_vec_t input_tensors = apParams_fb_FBNetwork_inputTensors(src_network);
        apParams_fb_FBOutputTensor_vec_t output_tensors = apParams_fb_FBNetwork_outputTensors(src_network);

        if (input_tensors) {
            dst_network->input_tensor_count = (uint8_t)imx500_min_size(IMX500_MAX_INPUT_TENSORS, apParams_fb_FBInputTensor_vec_len(input_tensors));
            for (uint8_t i = 0; i < dst_network->input_tensor_count; ++i) {
                apParams_fb_FBInputTensor_table_t src_tensor = apParams_fb_FBInputTensor_vec_at(input_tensors, i);
                if (!src_tensor) {
                    return false;
                }
                if (!imx500_parse_tensor_common(&dst_network->input_tensors[i],
                                                apParams_fb_FBInputTensor_id(src_tensor),
                                                apParams_fb_FBInputTensor_name(src_tensor),
                                                apParams_fb_FBInputTensor_shift(src_tensor),
                                                apParams_fb_FBInputTensor_scale(src_tensor),
                                                apParams_fb_FBInputTensor_format(src_tensor),
                                                8,
                                                apParams_fb_FBInputTensor_dimensions(src_tensor))) {
                    return false;
                }
            }
        }
        ...
        if (output_tensors) {
            dst_network->output_tensor_count = (uint8_t)imx500_min_size(IMX500_MAX_OUTPUT_TENSORS, apParams_fb_FBOutputTensor_vec_len(output_tensors));
            for (uint8_t i = 0; i < dst_network->output_tensor_count; ++i) {
                apParams_fb_FBOutputTensor_table_t src_tensor = apParams_fb_FBOutputTensor_vec_at(output_tensors, i);
                if (!src_tensor) {
                    return false;
                }
                if (!imx500_parse_tensor_common(&dst_network->output_tensors[i],
                                                apParams_fb_FBOutputTensor_id(src_tensor),
                                                apParams_fb_FBOutputTensor_name(src_tensor),
                                                apParams_fb_FBOutputTensor_shift(src_tensor),
                                                apParams_fb_FBOutputTensor_scale(src_tensor),
                                                apParams_fb_FBOutputTensor_format(src_tensor),
                                                apParams_fb_FBOutputTensor_bitsPerElement(src_tensor),
                                                apParams_fb_FBOutputTensor_dimensions(src_tensor))) {
                    return false;
                }
            }
        }
...
{
    IMX500OutputHeader primary_header = {0};
    IMX500OutputHeader output_header = {0};
...
bool parse_ap_params(const uint8_t* data, size_t data_len, DetectionResult* detection_result) {
    ...
    apParams_fb_FBNetwork_table_t network = apParams_fb_FBNetwork_vec_at(networks, 0);
    if (!network) {
        printf("network[0] is null\n");
        return false;
    }

    apParams_fb_FBOutputTensor_vec_t output_tensors = apParams_fb_FBNetwork_outputTensors(network);
    if (!output_tensors) {
        printf("outputTensors is null (schema mismatch or field missing)\n");
        return false;
    }

    if (apParams_fb_FBOutputTensor_vec_len(output_tensors) < 4) {
        printf("OutputTensor num is insufficient: %lu\n", (unsigned long)apParams_fb_FBOutputTensor_vec_len(output_tensors));
        return false;
    }
    ...
    if (apParams_fb_FBOutputTensor_vec_len(output_tensors) > MAX_OUTPUT_TENSOR_NUM) {
        printf("OutputTensor num is too large: %lu\n", (unsigned long)apParams_fb_FBOutputTensor_vec_len(output_tensors));
        return false;
    }

    const uint8_t *output_tensor_ptrs[MAX_OUTPUT_TENSOR_NUM] = {0};
    uint32_t output_tensor_sizes[MAX_OUTPUT_TENSOR_NUM] = {0};

    uint32_t output_data_offset = 0;

    for (uint32_t i = 0; i < apParams_fb_FBOutputTensor_vec_len(output_tensors); ++i) {
        apParams_fb_FBOutputTensor_table_t t = apParams_fb_FBOutputTensor_vec_at(output_tensors, i);
        if (!t) {
            printf("output_tensors[%lu] is null\n", i);
            return false;
        }

        apParams_fb_FBDimension_vec_t dims = apParams_fb_FBOutputTensor_dimensions(t);
        if (!dims || apParams_fb_FBDimension_vec_len(dims) == 0) {
            printf("tensor[%lu] dims is null/empty\n", i);
            return false;
        }

        uint32_t tensor_elements = 1;
        for (uint32_t j = 0; j < apParams_fb_FBDimension_vec_len(dims); ++j) {
            apParams_fb_FBDimension_table_t d = apParams_fb_FBDimension_vec_at(dims, j);
            if (!d) {
                printf("tensor[%lu] dim[%lu] is null\n", i, j);
                return false;
            }
            uint32_t s = (uint32_t)apParams_fb_FBDimension_size(d);
            if (s == 0) {
        ...
        uint8_t bits_per_element = apParams_fb_FBOutputTensor_bitsPerElement(t);
    ...
    apParams_fb_FBOutputTensor_table_t bbox_tensor = apParams_fb_FBOutputTensor_vec_at(output_tensors, 0);
    apParams_fb_FBOutputTensor_table_t score_tensor = apParams_fb_FBOutputTensor_vec_at(output_tensors, 1);
    apParams_fb_FBOutputTensor_table_t class_tensor = apParams_fb_FBOutputTensor_vec_at(output_tensors, 2);
    const int16_t* bbox_data = (const int16_t*)output_tensor_ptrs[0];
    const uint8_t* score_data = (const uint8_t*)output_tensor_ptrs[1];
    const int16_t* class_data = (const int16_t*)output_tensor_ptrs[2];
    const int16_t* detect_num_data = (const int16_t*)output_tensor_ptrs[3];
    ...
    BBox *bboxs = detection_result->bboxs;
    detection_result->valid_num = 0;

    const float confidence_threshold = 0.1f;
    for (uint32_t i = 0; i < max_items; ++i) {
        float confidence = ((float)score_data[i] - apParams_fb_FBOutputTensor_shift(score_tensor)) * apParams_fb_FBOutputTensor_scale(score_tensor);
        if (confidence < confidence_threshold) continue;

        float xmin = ((float)bbox_data[i] - apParams_fb_FBOutputTensor_shift(bbox_tensor)) * apParams_fb_FBOutputTensor_scale(bbox_tensor);
        float ymin = ((float)bbox_data[i + bbox_stride] - apParams_fb_FBOutputTensor_shift(bbox_tensor)) * apParams_fb_FBOutputTensor_scale(bbox_tensor);
        float xmax = ((float)bbox_data[i + bbox_stride * 2] - apParams_fb_FBOutputTensor_shift(bbox_tensor)) * apParams_fb_FBOutputTensor_scale(bbox_tensor);
        float ymax = ((float)bbox_data[i + bbox_stride * 3] - apParams_fb_FBOutputTensor_shift(bbox_tensor)) * apParams_fb_FBOutputTensor_scale(bbox_tensor);
        uint32_t class_id = (uint32_t)(((float)class_data[i] - apParams_fb_FBOutputTensor_shift(class_tensor)) * apParams_fb_FBOutputTensor_scale(class_tensor));
...
bool get_spi_flash_status(spi_flash_status_t *status) {
    ...
    for (uint32_t attempt = 0; attempt < SPI_FLASH_STATUS_READ_RETRIES; ++attempt) {
        spi_flash_status_t local_status = {0};
...
static bool wait_for_spi_flash_terminal_with_poll(uint32_t timeout_ms,
                                                  uint32_t poll_interval_ms,
                                                  spi_flash_status_t *status_out) {
    spi_flash_status_t status = {0};
    uint32_t elapsed_ms = 0;
    if (poll_interval_ms == 0u) {
        poll_interval_ms = SPI_FLASH_POLL_INTERVAL_MS;
    }
    while (elapsed_ms < timeout_ms) {
        if (!get_spi_flash_status(&status)) {
            if (g_i2c_driver.slp_ms) {
...
static bool wait_for_spi_flash_terminal(uint32_t timeout_ms,
                                        spi_flash_status_t *status_out) {
    return wait_for_spi_flash_terminal_with_poll(timeout_ms,
                                                 SPI_FLASH_POLL_INTERVAL_MS,
                                                 status_out);
}
...
static bool wait_for_spi_flash_receiver(uint32_t bytes_done,
    ...
    while (elapsed_ms < timeout_ms) {
        ...
        if (status.status == SPI_FLASH_OP_RECEIVING &&
            ...
            spi_flash_status_t confirm_status = {0};
...
static bool sdk_i2c_write_payload_chunk_retry(const uint8_t *buf,
                                              uint32_t len,
                                              uint32_t timeout_ms) {
    uint32_t elapsed_ms = 0;
    while (elapsed_ms <= timeout_ms) {
        if (sdk_i2c_write_payload_chunk(buf, len) >= 0) {
            return true;
        }
...
static bool run_i2c_blob_transfer(uint32_t operation,
    ...
    uint8_t header[sizeof(SpiBlobWireHeader)] = {0};
    ...
    while (stream_sent < stream_size) {
        ...
        uint32_t payload_after =
            stream_sent + chunk > sizeof(SpiBlobWireHeader)
                ? stream_sent + chunk - (uint32_t)sizeof(SpiBlobWireHeader)
                : 0u;
        if (payload_after > payload_size) {
            payload_after = payload_size;
        }
        ...
        stream_sent += chunk;
        uint32_t payload_sent = payload_after;
...
bool reset_imx500_module(void) {
    return reset_imx500_module_to_loader_main_ready(NULL);
}
...
bool imx500_open(const uint8_t *nn_fw, uint32_t nn_fw_size, const uint8_t* nn_info, uint32_t nn_info_size, mipi_data_format_t mipi_format, spi_data_format_t spi_format, uint32_t fps) {
    ...
    uint32_t imx500_boot_status = 0;
    const bool direct_boot = (nn_fw != NULL && nn_fw_size > 0u);
    ...
    cmd_err = imx500_res_read(IMX500_COMMAND_SENSOR_DEFAULT_CONFIG, &val, 5000);
    if (cmd_err != IMX500_CMD_OK) {
